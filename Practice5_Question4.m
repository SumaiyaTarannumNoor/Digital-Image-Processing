...
for f = 1:filn
   ...
   F_deconv = Fout ./ (F_fil + eps);
   deconv_im = real(ifft2(F_deconv));
   deconv_im = deconv_im(1:r, 1:c);
  

   figure;
   subplot(1,4,1); imshow(uint8(im)); title(['Original Image (n = ', num2str(n), ')']);
   subplot(1,4,2); imshow(uint8(fspatialim)); title(['Spatial Filtering (n = ', num2str(n), ')']);
   subplot(1,4,3); imshow(uint8(filim_freq)); title(['Frequency Filtering (n = ', num2str(n), ')']);
   subplot(1,4,4); imshow(uint8(deconv_im)); title(['Deconvolved Image (n = ', num2str(n), ')']);
end
